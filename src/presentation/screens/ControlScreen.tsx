...
import { Alert, View, ActivityIndicator, FlatList, TouchableOpacity, StyleSheet, Image } from "react-native"
...
// Importar SignalR para React Native
...
const ControlAlert: React.FC<ControlAlertProps> = ({
  ...
  const hasTriggeredExpiredRef = useRef(false) // Renamed for clarity: tracks if onTimeExpired has been called for current cycle

  // Effect to initialize totalSeconds and manage the countdown interval
  useEffect(() => {
    console.log(
    ...
    // Siempre sincronizar totalSeconds con la prop initialSeconds
    ...
    // Resetear el flag de navegación si initialSeconds cambia a un valor positivo
    // Esto es crucial para que el cronómetro pueda volver a activarse si se recibe una nueva fechaini futura
    if (initialSeconds > 0) {
    ...
    if (showTimer) {
      if (initialSeconds > 0) {
        // Start countdown only if it hasn't already triggered for this cycle
        if (!hasTriggeredExpiredRef.current) {
          intervalRef.current = setInterval(() => {
            setTotalSeconds((prevSeconds) => {
              ...
              if (newSeconds <= 0) {
                console.log("[ControlAlert Effect] Countdown finished. Clearing interval.")
                if (intervalRef.current) {
                  clearInterval(intervalRef.current)
                  intervalRef.current = null
                }
        ...
        // initialSeconds es 0 o negativo, y showTimer es true (navegación inmediata)
        if (!hasTriggeredExpiredRef.current) {
          // Prevent double call
          console.log(
          ...
          hasTriggeredExpiredRef.current = true
          onTimeExpired?.()
          onTimeChange?.(0) // Asegurar que el padre sepa que es 0
        }
      }
    }
  ...
  }, [initialSeconds, showTimer, onTimeChange, onTimeExpired]) // Dependencies

  // Calculate minutes and seconds
  const minutes = Math.floor(totalSeconds / 60)
  const seconds = totalSeconds % 60

  // Determine the background color and status
  const getAlertStyle = () => {
    if (!isRutaActive) {
      // No hay despacho - GRIS
      return {
    ...
    >
      ...
      >
        <View style={{ flexDirection: "row", alignItems: "center", flex: 1 }}>
          <IonIcon name={alertStyle.icon} size={26} color={alertStyle.iconColor} />
...
export const ControlScreen = () => {
  ...
  const [alertSeconds, setAlertSeconds] = useState(60) // This state is updated by ControlAlert via onTimeChange
const [isScreenActive, setIsScreenActive] = useState(true)
  ...
  const [signalRData, setSignalRData] = useState<{
    deviceID: string
    fechaini: string | null // fechaini can be null
    isruta: string
  } | null>(null)
  const [timerDuration, setTimerDuration] = useState(0)
  const [showTimer, setShowTimer] = useState(false)
  const [isRutaActive, setIsRutaActive] = useState(false)
  const [signalRDataUpdateCount, setSignalRDataUpdateCount] = useState(0) // Nuevo contador para la key
  const [isSignalRDataLoaded, setIsSignalRDataLoaded] = useState(false) // Nuevo estado para controlar la carga inicial de SignalR

  // Función para calcular la diferencia de tiempo en segundos
  const calculateTimeDifference = useCallback((fechaini: string | null): number => {
    // fechaini can be null
    if (!fechaini || typeof fechaini !== "string" || !fechaini.includes(":")) {
      console.log(`[calculateTimeDifference] fechaini es nulo o inválido: ${fechaini}. Retornando 0 segundos.`)
      return 0 // Return 0 if fechaini is null, undefined, not a string, or doesn't contain ':'
    }

    const now = new Date()
    const [targetHours, targetMinutes] = fechaini.split(":").map(Number)

    // Crear un objeto Date para la hora objetivo HOY
    const targetDateToday = new Date(now.getFullYear(), now.getMonth(), now.getDate(), targetHours, targetMinutes, 0, 0)

    const diffInMilliseconds = targetDateToday.getTime() - now.getTime()

    console.log(`[calculateTimeDifference] Hora actual: ${now.toLocaleTimeString()}`)
    console.log(`[calculateTimeDifference] Hora objetivo (fechaini): ${fechaini}`)
    console.log(`[calculateTimeDifference] Diferencia en milisegundos (targetDateToday - now): ${diffInMilliseconds}`)

    // Si la hora objetivo ya pasó o es la actual (diffInMilliseconds <= 0),
    // significa que no hay tiempo de espera, el cronómetro debe ser 0.
    if (diffInMilliseconds <= 0) {
      console.log(`[calculateTimeDifference] La hora objetivo ya pasó o es la actual. Retornando 0 segundos.`)
      return 0
    }

    // Si la hora objetivo está en el futuro, calcular la diferencia en segundos.
    // Redondear hacia arriba para asegurar que el cronómetro se muestre si hay segundos restantes.
  ...
  // Función de navegación, memoizada para useFocusEffect
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          setDialogMessage("La placa ingresada por el usuario es incorrecta, vuelva a intentarlo.")
        } else {
  ...
  }, [textPlaca, isRutaActive, navigation]) // Dependencias para useCallback
  ...
  useEffect(() => {
    const initializeSignalR = async () => {
      try {
        ...
        // Configurar el evento para recibir datos
        newConnection.on("ActualizarDatosUrbano", (data) => {
          console.log("Datos recibidos de SignalR:", data)
          setSignalRData(data) // Mantener este estado para la dependencia del useFocusEffect y la key
          setIsRutaActive(data.isruta === "1") // Actualizar isRutaActive directamente

          // Pasar data.fechaini directamente a calculateTimeDifference, que ahora maneja null
          const timeDiffSeconds = calculateTimeDifference(data.fechaini)
          console.log(`[SignalR Handler] Diferencia de tiempo calculada (segundos): ${timeDiffSeconds}`)

          // Siempre mostrar el cronómetro si hay una ruta activa, incluso si el tiempo es 0
          setShowTimer(true) // Actualizar showTimer directamente
          setTimerDuration(timeDiffSeconds) // Actualizar timerDuration directamente
          setSignalRDataUpdateCount((prev) => prev + 1) // Incrementar contador para forzar re-montaje de ControlAlert
          setIsSignalRDataLoaded(true) // Marcar que los datos iniciales de SignalR han sido cargados

          console.log("[SignalR Handler] Cronómetro configurado. La navegación se activará al expirar el cronómetro.")
        })
  ...
  useEffect(() => {
    if (connection && connection.state === HubConnectionState.Connected && textPlaca.trim() !== "") {
      console.log(`Uniéndose al grupo: ${textPlaca}`)
      connection
        .invoke("UnirGrupo", textPlaca.trim())
        .catch((error: any) => console.error("Error uniéndose al grupo:", error))
    }
...
useFocusEffect(
  React.useCallback(() => {
    ...
    return () => {
      console.log("[useFocusEffect Cleanup] Pantalla perdiendo foco. Desactivando componente.")
...
const handleTimeExpired = useCallback(() => {
  console.log("[handleTimeExpired] Cronómetro en ControlAlert ha llegado a cero.")
  
  // AGREGAR ESTA VERIFICACIÓN
  if (!isScreenActive) {
    console.log("[handleTimeExpired] Pantalla no activa. Cancelando navegación.")
    return
  }
  
  if (isRutaActive) {
    console.log("[handleTimeExpired] isRutaActive es true. Llamando a handleNavigation.")
    handleNavigation()
  } else {
    console.log("[handleTimeExpired] isRutaActive es false. No se navega.")
  }
}, [isRutaActive, handleNavigation, isScreenActive]) // AGREGAR isScreenActive aquí
  ...
  const enviarAsignacionIMEI = async () => {
    ...
    try {
      const response = await fetch("https://villa.velsat.pe:8443/api/Datero/asignar", {
        method: "POST",
        headers: {
  ...
  const fetchDeviceIDs = async () => {
    try {
      const response = await fetch("https://villa.velsat.pe:8443/api/Datero/devices/transporvilla")
      if (!response.ok) {
  ...
  const handleTextChange = (text: string) => {
    setTextPlaca(text)
    if (text.length > 0) {
      const filtered = deviceIDs.filter((deviceID) => deviceID.toLowerCase().includes(text.toLowerCase()))
  ...
  // Lógica para determinar si el botón de navegación debe estar habilitado
  // El botón solo se habilita si no hay cronómetro activo (showTimer es false)
  const canNavigateManually = isRutaActive && !showTimer && !loading

  // Lógica para determinar el color del botón
  const getButtonColor = () => {
    if (loading) return "#e6a000" // Cargando
    if (!isRutaActive) return "#6c757d" // Sin despacho
    if (showTimer) return "#ffc107" // Cronómetro activo, esperando
    return "#ffb703" // Listo para navegar
  }
  ...
  const getButtonText = () => {
    ...
    if (showTimer) return "Esperando.." // Nuevo texto para cuando el cronómetro está activo
    return "Mostrar Ruta"
  }
  ...
  return (
    <View style={globalStyles.container}>
      <Portal>
        ...
        <Dialog visible={dialogVisible} onDismiss={hideDialog} style={styles.dialogContainer}>
          <Dialog.Title style={styles.dialogTitle}>Aviso</Dialog.Title>
          <Dialog.Content>
            <Text style={styles.dialogMessage}>{dialogMessage}</Text>
          </Dialog.Content>
          <Dialog.Actions>
            <Button onPress={hideDialog} mode="contained" style={styles.confirmButton}>
              Entendido
            </Button>
          </Dialog.Actions>
        </Dialog>
      ...
      <View style={{ flexDirection: "row", flex: 1 }}>
        ...
        >
          <View style={{ marginTop: 20 }}>
            <Text style={{color:'#fff', fontSize:18, fontWeight:'bold', alignSelf:'center'}}>BIENVENIDO A</Text>
          </View>
          <Image
  ...
  <View style={{ height: 75, justifyContent: "center", alignItems: "center" }}>
    ...
    <Text style={{ color: "#FFF", marginTop: 10 }}>
      {!isSignalRDataLoaded ? "Cargando datos de despacho..." : "Pantalla inactiva"}
    </Text>
  </View>
)}
          </View>
          <View style={globalStyles.containerControlText}>
            <Text style={{color:"#fff", fontSize:12}}>Lima - Perú</Text>
            <Text style={{color:"#fff",fontSize:12}}>© 2025 VELSAT SAC</Text>
          </View>
          <View style={{ padding: 5 }}>
            ...
            >
              <View style={{ flexDirection: "row" }}>
                <Text style={{ marginRight: 10, color: "#FFF", fontWeight: "bold" }}>Asignar IMEI</Text>
        ...
        >
          ...
          <View style={{ flexDirection: "row", alignItems: "center", marginTop: 8 }}>
            ...
            >
              ...
              >
                {loading && <ActivityIndicator color="#212529" style={{ marginRight: 5 }} />}
                ...
                {!loading &&
                  ...
                  !showTimer && ( // Muestra el icono solo cuando está listo para navegar
                    <IonIcon name="map" color="#212529" size={20} style={{ marginLeft: 5 }} />
                  )}
              </View>
            </Button>
